...
Scoring rules:
- intent_correct      : 1 if the bot routed to the right type of agent/tool, else 0
- resolution_achieved : 1 if the customer's issue was clearly resolved or a concrete next step promised, else 0
- tone_appropriate    : 1-5 (5 = very empathetic and polite, 1 = rude or dismissive)
- language_correct    : 1 if the bot responded in the SAME language as the customer (or English if customer used English), else 0
...
async def _evaluate_conversation(
    ...
    # ── Upload image if damaged_product and image_path exists ────────────────
    uploaded_image_path: str | None = None
    if scenario == "damaged_product" and image_path:
        abs_path = Path(image_path)
        if not abs_path.is_absolute():
            # Try relative to repo root (two levels above backend/)
            abs_path = BACKEND_DIR.parent / image_path
        if abs_path.exists():
            try:
                with abs_path.open("rb") as img_f:
                    resp = await http_client.post(
                        IMG_ENDPOINT,
                        params={"session_id": session_id},
                        files={"file": (abs_path.name, img_f, "image/jpeg")},
                        timeout=30,
                    )
                if resp.status_code == 200:
                    uploaded_image_path = resp.json().get("image_path")
                    image_uploaded = True
                    print(f"  image uploaded → {uploaded_image_path}", flush=True)
                else:
                    print(f"  image upload failed ({resp.status_code})", flush=True)
            except Exception as exc:
                print(f"  image upload error: {exc}", flush=True)
        else:
            print(f"  image_path not found on disk, skipping upload", flush=True)
...
def _build_report(results: list[dict[str, Any]], elapsed_sec: float) -> str:
    n = len(results)
    if n == 0:
        return "No results to report.", "# No results to report."
    ...
    for sc, st in sorted(scenario_stats.items()):
        sc_lines.append(
            f"| {sc:<20} | {st['n']:>5} | "
            f"{st['resolved']/st['n']*100:>6.1f}% | "
            f"{st['intent']/st['n']*100:>6.1f}% |"
        )
...
async def main(count: int) -> None:
    ...
    terminal_report, md_report = _build_report(all_results, elapsed)
